...
def int_erf_AX_sin_Bx(A,B,h,h1):
    T1 = -1/B*np.cos(B*h)*scipy.special.erf(A*h) + 1/(2*B)*np.exp(-np.power(B,2)/(4*np.power(A,2)))*np.real(scipy.special.erf(A*h-B/(2*A)*1j)+scipy.special.erf(A*h+B/(2*A)*1j))
    T2 = -1/B*np.cos(B*h1)*scipy.special.erf(A*h1) + 1/(2*B)*np.exp(-np.power(B,2)/(4*np.power(A,2)))*np.real(scipy.special.erf(A*h1-B/(2*A)*1j)+scipy.special.erf(A*h1+B/(2*A)*1j))
    return T1-T2
    
def int_erf_AX_cos_Bx(A,B,h,h1):
    T1 = 1/B*np.sin(B*h)*scipy.special.erf(A*h) + np.real(1j/(2*B)*np.exp(-np.power(B,2)/(4*np.power(A,2)))*(scipy.special.erf(A*h-B/(2*A)*1j)-scipy.special.erf(A*h+B/(2*A)*1j)))
    T2 = 1/B*np.sin(B*h1)*scipy.special.erf(A*h1) + np.real(1j/(2*B)*np.exp(-np.power(B,2)/(4*np.power(A,2)))*(scipy.special.erf(A*h1-B/(2*A)*1j)-scipy.special.erf(A*h1+B/(2*A)*1j)))
    return T1-T2
...
def int_second_part2(Dc_prior,D1,D2,lam,h1,h2,t):
    ...
    B = lam/np.power(D2,0.5)
    h = h2+h1
    term1 = np.cos(B*h)*int_erf_AX_cos_Bx(A,B,h,h1)
    term2 = np.sin(B*h)*int_erf_AX_sin_Bx(A,B,h,h1)
...
def C_paper_vectorized2(cover_2,mortar,h2,D1,D2_ini,D2_rep,trep,T,Cs,nmax,interval):
    
    som = Cs.copy()
    lambdas = search_lambda5_vect(mortar/np.power(D1,0.5),h2/np.power(D2_rep,0.5),D1,D2_rep,nmax,interval)#*(h1/np.power(D1_single,0.5)))
    ...
    BB = total_integral(Dc_prior,D11,D22,lambdas,mortar,h2,trep,Cs_extend).T

    D1_ext = np.tile(D1[:,np.newaxis],(1,nmax))
    D2_ext = np.tile(D2_rep[:,np.newaxis],(1,nmax))
    integral_term = integral(mortar,h2,D1_ext,D2_ext,lambdas).T  
  
    Term2 = fn(np.tile(cover_2,(nmax,1)),mortar,h2,np.tile(D1,(nmax,1)),np.tile(D2_rep,(nmax,1)),lambdas.T)
    
    som = Cs+np.sum(BB/integral_term*Term2*np.exp(-np.power(lambdas.T,2)*T),axis=0)
    
    return np.maximum(som,0.0)

def mortar_replacement(cover,mortar,h2,D1,D2_ini,D2_rep,trep,T,Cs,Ccr,nmax,interval):
    #plt.plot(C_paper_vectorized(cover,h1,h2,D1,D2,trep,T,Cs,nmax,interval,a_c,b_c,a_m))
    a = C_paper_vectorized2(cover,mortar,h2,D1,D2_ini,D2_rep,trep,T,Cs,nmax,interval)
    return sum(np.where(a>Ccr,1,0))/len(cover)

def C_paper_vectorized2_over_time(cover_2,mortar,h2,D1,D2_ini,D2_rep,trep,T,Cs,nmax,interval):
...
def mortar_replacement_over_time(cover,mortar,h2,D1,D2_ini,D2_rep,trep,T,Cs,Ccr,nmax,interval):
    #plt.plot(C_paper_vectorized(cover,h1,h2,D1,D2,trep,T,Cs,nmax,interval,a_c,b_c,a_m))
    
    res_list = C_paper_vectorized2_over_time(cover,mortar,h2,D1,D2_ini,D2_rep,trep,T,Cs,nmax,interval)
    Pdep_list = []
    for res in res_list:
...
def original_c_profile(Cs,Dc,cover,t):
    C_ini = Cs*(1-scipy.special.erf(cover/(2*np.power(Dc*t,0.5))))
    return C_ini
'''
...
nmax = 30
interval = 100
D2_total = np.array([100000])
a = 0.5
xsp = np.linspace(0,100,100)
ll = []
for x in xsp:
    L = C_paper_vectorized2(np.array([x]),h1,h2,D1,D2_ini,D2_rep,trep,T,Cs,nmax,interval,D2_total,a)
    ll.append(L)
plt.plot(xsp,ll)
'''
...
C_paper_vectorized2(samplescover,20,300,samplesDmort,samplesDconcr,samplesDconcr,20,50,samplesCs,10,20)
